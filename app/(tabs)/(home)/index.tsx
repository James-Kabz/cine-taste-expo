...
import {  StyleSheet, FlatList, View, TouchableOpacity, ActivityIndicator, ScrollView } from 'react-native';
...
export default function HomeScreen() {
  ...
  const handleAddToWatchlist = useCallback(async (movieId: number) => {
    if (!session) return;
    ...
    try {
      const response = await fetch('https://cinetaste-254.vercel.app/api/watchlist', {
        ...
        body: JSON.stringify({ movieId, sendEmail: false }),
      });
  ...
  const renderMovieItem = useCallback(({ item }: { item: Movie }) => (
    <TouchableOpacity 
      ...
      onPress={() => router.push(`/(tabs)/(home)/movies/${item.id}`)}
    >
      <Image
    ...
    return (
      <ThemedView style={styles.errorContainer}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => fetchMovies(activeCategory, currentPage)}
        >
